...
public:
    ...
    void gelistir() {
        gelistirmeSeviyesi++;
        benzinTuketimi *= 0.9; // Yakýt tüketimi %10 azalýr
        hizLimiti += 5; // Hýz limiti artar
...
public:
    ...
    Oyuncu(string n, string kt)
        : isim(n), karakterTuru(kt), para(0), puan(0), toplamMesafe(0), arac(nullptr), yarisAlani(nullptr) {}
...
public:
    ...
    void oyunuBaslat() {
        ...
        cin >> isim;
        cout << "Iyi bir karakter misiniz yoksa kotu mu? (iyi/kotu): ";
        cin >> karakterTuru;
        oyuncu = new Oyuncu(isim, karakterTuru);
        cout << "\nHos geldiniz, " << isim << "! Macera baslamak uzere...\n\n";
    }
    ...
    void raporHazirla(int gorev, double kalanBenzin, int puan, double toplamMesafe) {
        cout << "\n======= Gunluk Rapor =======\n";
        cout << "Gorev: " << gorev << "\n";
    ...
    void yarisiBaslat() {
        ...
        for (int i = 1; i <= 20; ++i) {
            ...
            if (karar == 1) {
                ...
                bildirim.durumGuncelle(bildirim.benzinDurumu - harcananBenzin);
                oyuncu->mesafeEkle(10);
            } else if (karar == 2) {
                cout << "Yakit ikmali yapiliyor...\n";
                bildirim.durumGuncelle(100);
            } else if (karar == 3) {
            ...
            raporHazirla(i, bildirim.benzinDurumu, oyuncu->puan, oyuncu->toplamMesafe);

            if (bildirim.benzinDurumu <= 20) {
                cout << "Uyari: Benzin az! Depoyu doldurmayi dusunun.\n";
            }

            if (bildirim.benzinDurumu <= 0) {
